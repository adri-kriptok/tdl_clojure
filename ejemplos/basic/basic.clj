;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; Adrián Cavaiuolo - 91186 (2do Cuatrimestre 2020)
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; Agrega un par de par譴esis extra a los MID3 para que no se
; mezclen los argumentos cuando tiene una expresi򬟣ompleja
; como "L - I + 1".
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defn desambiguar-mid3
  ([expr]
    (cond
      (contains? (set expr) 'MID3$) (desambiguar-mid3 expr 0 (count expr) 0 0 0 true)      
      :else (apply list expr)
    )
  )
  ([expr act fin pos cont-paren cont-comas buscando]
    (if (= act fin)
      (desambiguar-mid3 expr)
      (let [nuevo (nth expr act)]
        (cond
          (and (= nuevo 'MID3$) buscando) 
            (recur expr (inc act) fin act cont-paren cont-comas false)
          (and (= nuevo (symbol "(")) (not buscando)) 
            (recur expr (inc act) fin pos (inc cont-paren) cont-comas buscando)
          (and (= nuevo (symbol ")")) (not buscando))
            (if (= cont-paren 1)              
              (recur (assoc (vec expr) pos 'MID6$) (inc act) fin 0 0 0 true)                  
              (recur expr (inc act) fin pos (dec cont-paren) cont-comas buscando))
          (and (= nuevo (symbol ",")) (= cont-paren 1)) 
            (do 
              ;(println "Encontre una coma, cont-comas " cont-comas ", act " act)
              ;(println "expr  " expr)
              ;(println "expr.1" (flatten (list (take (+ act 1) expr) (symbol "(") (drop (+ act 1) expr))))
              ;(println "expr.2" (flatten (list (take act expr) (symbol ")") (drop act expr))))
              (let [newExpr
                (if (= cont-comas 0)
                  (flatten (list (take (+ act 1) expr) (symbol "(") (drop (+ act 1) expr)))
                  (flatten (list (take act expr) (symbol ")") (drop act expr)))
                ), 
                newAct (+ act (if (= cont-comas 0) 1 1)), 
                newFin (+ fin (if (= cont-comas 0) 1 1)), 
                newPos (+ pos (if (= cont-comas 0) 0 0))]
                (do
                   ;(println "newExpr " newExpr)  
                   (recur newExpr (inc newAct) newFin newPos cont-paren (inc cont-comas) buscando)
                )
              )              
            )
            :else (recur expr (inc act) fin pos cont-paren cont-comas buscando)
...
(defn evaluar [sentencia amb]
  (if (or (contains? (set sentencia) nil) (and (palabra-reservada? (first sentencia)) (= (second sentencia) '=)))
      ...
      (case (first sentencia)
        ...
        END [nil amb]
        READ (leer-data (drop 1 sentencia) amb)       
        ...
        (case operador
          ...
          * (* operando1 operando2)
          ...
          MID$ (if (< operando2 1)
            ...
            (let [ini (dec operando2)] 
               (if (>= ini (count operando1)) "" (subs operando1 ini))
            )
          )
          
          LEFT$ (if (< operando2 1) 
            (dar-error 53 nro-linea)  ; Illegal quantity error                        
            (if (> operando2 (count operando1))
              operando1
              (subs operando1 0 operando2)              
            )
          )
        )        
      )
...
(defn cargar-linea [linea amb]		
  ; Reemplazo en el ambiente la lista
	(assoc amb 0 
		; Ordeno la lista modificada	
		(sort-by (fn [x] (first x)) 
		; Agrego el elemento.
			(conj 
				; Filtro por si ya está cargado ese numero de linea	
				(filter (fn [x] (distinct? (first x) (first linea)))
...
(defn expandir-nexts [n] 
	; Concateno la lista de listas
	(cond
	  (= n nil) nil
	  :else(apply concat 		
  		(map (fn [x] (
  			; Si es un NEXT
  			cond (and (= (first x) 'NEXT) (> (count x) 1))
  				; Hago lo necesario.
  				(map (fn [a] (list (first x) a)) (filter (fn [x] (distinct? x (symbol ","))) (drop 1 x)))
  			; De lo contrario, devuelvo el objeto pero en una lista
  			:else (list x)
  	  )) n)
    )
  )
...
(defn dar-error [cod prog-ptrs]
  (if (number? cod)
    (if (number? (first prog-ptrs))
      (print (buscar-mensaje cod) "IN" (first prog-ptrs))
      (print (buscar-mensaje cod)))
    (if (number? (first prog-ptrs))
      (print cod "IN" (first prog-ptrs))
      (print cod))
  )
  nil
)
...
(defn contar-sentencias [nro-linea amb]
  (let [prg     (amb 0),
        line    (first (filter (fn [x] (= (first x) nro-linea)) prg)),
        droped  (drop 1 line)
        expLine (expandir-nexts droped)]   
    (count expLine)    
  )
)
...
(defn buscar-lineas-restantes
  ...
  ([act prg]
    (cond
      ; Si no hay nada cargado en el programa, directamente salgo.
      (keyword? (first act)) nil
      (= 0 (count prg)) nil
      :else(let[numLine (first act), numInstr (second act), maxLine (apply max (map (fn [x] (first x)) prg))]                
          (cond 
            ; Primero valido que el puntero no este fuera del programa.
            (> numLine maxLine) nil
            :else (map (fn [x] 
              (cond
                (= numLine (first x))
                  ; Si es la linea que estoy buscando
                  ; Tengo que fijarme el numero de instruccion.
                  (cond
                    ; Si el numero de instruccion es menor o igual a cero, solo devuelvo el numero
                    (<= numInstr 0) (list numLine)
                    :else
                      ; De lo contrario, tomo todas las instrucciones restantes desde la izquierda.
                
                      ; Lo vuelvo a juntar con el numero de linea
                      (concat (list numLine)
                        ; Lo vuelvo a enderezar.
                        (reverse 
                          ; Tomo los que necesito.
                          (take numInstr
                            ; Lo invierto para poder tomar los que necesito.
                            (reverse 
                              ; Saco el primero que es el numero
                              (expandir-nexts (drop 1 x))
                            )
                          )
                        )
                      )
                ...
                ;(> numLine (first x))
                ;  ; Para las lineas que son menores, solo dejo el numero de linea
                ;  (list (first x))
                ; Sino, devuelvo la linea como estaba.       
                :else x            
              )
            ) (filter (fn [x]
              (>= (first x) numLine)          
...
(defn continuar-linea [amb]  
  (let [ptrs (amb 2)]
    (if (empty? ptrs)
      ...
      (let [lst (last ptrs)]        
        (if (> (count ptrs) 1)
          (dar-error "Continuar-linea - ERROR, hay mas de uno" (amb 1))
          (let [lst2 (assoc lst 1 (- (lst 1) 1))]              
            [:omitir-restante (assoc (assoc amb 1 lst2) 2 [])]              
          )          
        )
      )
    )
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; extraer-data: recibe la representaci򬟩ntermedia de un programa
...
(defn extraer-data [prg]
  ; Y convierto los simbolos a string
  (map (fn [x] (if (symbol? x) (str x) x ))
  ; Filtro las comas.
  (filter (fn [x] (distinct? x (symbol ",")))
	  ; Aplano.
		(flatten
			; Remuevo los DATA
			(map (fn [x] (drop 1 x))
			; Tomo todo lo que empiece con DATA.
			(filter (fn [x] (= (first x) 'DATA))
				; Concateno resultados
				(apply concat 
					(map (fn [x] (
						; Tomo todo antes de los "rem".
						take-while (fn[y] (distinct? (first y) 'REM)) 
						; Remuevo los numeros del linea.
							(drop 1 x)
					)) prg)
				)
			)	
...
(defn ejecutar-asignacion [sentencia amb]
  (let [var (first sentencia), body (drop 2 sentencia), vars (last amb)]           
    (let [result (calcular-expresion  body amb),
          casted (if (and (variable-string? var) (not (string? result))) (str result) result)]
      (let [newVars (assoc vars var casted)]
          (assoc amb 6 newVars)
      )        
    )    
  )
...
(defn preprocesar-expresion [expr amb]  
    (cond               
      ; Si tiene puntos y comas, lo resuelvo afuera.
      (empty? expr) (list (symbol ""))
      (= (first expr) (symbol ";")) expr
      (= expr (list nil)) (list (symbol ""))
      :else(do    
        (let [vars (last amb)]        
            (map (fn [x]
          		(cond          				
          			(contains? vars x)    (get vars x)
          			(variable-string? x)  ""          
...
(defn eliminar-cero-decimal [n]  
  (if (number? n)(if (zero? (- n (int n))) (int n) n ) n)
)
...
(defn eliminar-cero-entero [n]
  (cond
    (= n nil) nil
    (symbol? n) (str n)
    (ratio? n) (recur (float n))
    :else
    (if (number? n)
      (cond
        (zero? n) (str n)
        :else(let [a (Math/abs n), i (int a)]
          (if(= i 0) 
            (if (neg? n) 
              (apply str (flatten (list "-" (rest (seq (str a))))))
              (apply str (rest (seq (str a))))          )
            (str n)
          )
        )
      )
